...
function generateComponentsStrings (filesTexts, config) {
  return new Promise((resolve, reject) => {
    ...
    filesTexts.forEach(svg => {
      ...
      if (config.typescript) {
        componentDeclarationString = strings.componentDeclarationTypescript(componentName)
      }
      else if (config.coffeescript) {
...
function generateIndexStrings (completedComponents, config) {
  ...
  completedComponents.forEach(componentInfo => {
    if (config.typescript) {
      componentImports.push(strings.componentsImportStringTypescript(componentInfo))
      componentRequires.push(strings.componentsRequireStringTypescript(componentInfo))
    } else if (config.coffeescript) {
      componentImports.push(strings.componentsImportStringOnCoffeescript(componentInfo))
      componentRequires.push(strings.componentsRequireStringOnCoffeescript(componentInfo))
    } else {
      componentImports.push(strings.componentsImportString(componentInfo))
      componentRequires.push(strings.componentsRequireString(componentInfo))
    }
  })

  if (config.typescript) {
    startImportString = strings.startImportStringTypescript()
  } else if (config.coffeescript) {
  ...
  componentsImportString = componentImports.join('')
  ...
  return writeComponentsIndex({
    ...
    componentsImportString: componentsImportString,
