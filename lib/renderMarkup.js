...
function iterateMarkup(markup, config, i = 0) {
  ...
  if (tagName === "svg") {
    let viewBox = markup.attributes
      ? filter(markup.attributes, attr => attr.name === "viewBox")[0]
      : false;
    ...
    attrs.push({
      name: "width",
      value: viewBox
        ? `{props.width || ${viewBox[2].replace(/\D/g, "")}}`
        : `{props.width}`
    });
    attrs.push({
      name: "height",
      value: viewBox
        ? `{props.height || ${viewBox[3].replace(/\D/g, "")}}`
        : `{props.height}`
    });
    attrs.push({
      name: "viewBox",
          ...
          forEach(rule.declarations, declaration => {
            ...
            if (config.reactNative) {
              ...
              if (validKeys.indexOf(propertyName) > -1) {
                ...
                } else if (config.color && propertyName === "stroke") {
                  propertyValue = `{props.strokeColor || "${declaration.value}"}`;
                } else {
    ...
    forEach(markup.attributes, attr => {
      ...
      if (config.color && propertyName === "fill") {
        ...
        propertyValue = `{props.strokeColor || "${attr.value}"}`;
      } else {
  ...
  const children = markup.childNodes.length
    ? map(markup.childNodes, child => {
  ...
  if (tagName === "Svg" || tagName === "Svg.Svg") {
    tag = `<${tagName} {...props} ${map(
      attrs,
      attr => `${attr.name}=${attr.value}`
    ).join(" ")}>${children}</${tagName}>`;
  } else {
    tag = `<${tagName} ${map(attrs, attr => `${attr.name}=${attr.value}`).join(
      " "
    )}>${children}</${tagName}>`;
  }
